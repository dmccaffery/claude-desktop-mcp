...
#
# What it does:
#   1. Builds the local server (`uv sync`) and health-checks all three modes.
#   2. Backs up your existing claude_desktop_config.json.
#   3. Swaps in a config with `fake-mcp-full`, `fake-mcp-search`, and
#      `fake-mcp-gateway` entries that run *this* working copy over stdio.
...
log "Health-checking all three modes…"
...
async def main():
    for mode in ("full", "search", "gateway"):
...
touch "$LOG_DIR/full.jsonl" "$LOG_DIR/search.jsonl" "$LOG_DIR/gateway.jsonl"
cat <<EOF

  Claude Desktop is now wired to:
    • fake-mcp-full     (108 tools, no search)        -> $LOG_DIR/full.jsonl
    • fake-mcp-search   (search only, catalog hidden)  -> $LOG_DIR/search.jsonl
...
tail -f "$LOG_DIR/full.jsonl" "$LOG_DIR/search.jsonl" "$LOG_DIR/gateway.jsonl"
